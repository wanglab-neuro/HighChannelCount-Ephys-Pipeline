function spikes=LoadSpikes_SpykingCircus(fileName)

fName=regexp(fileName,'\S+?(?=\.\w+\.\w+$)','match','once');
    postFix='';
    if isempty(fName)
        fName=regexp(fileName,'\S+?(?=\.\w+\-\w+\.\w+$)','match','once'); %in case loading merged files
        if ~isempty(fName)
            postFix='-merged';
        end
    end
